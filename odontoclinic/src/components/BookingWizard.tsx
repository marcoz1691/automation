...
export function BookingWizard() {
  ...
  const [availability, setAvailability] = useState<
    { dentistId: string; dentistName: string; slots: string[] }[]
  >([]);
  ...
  if (success) {
    return (
      <div className="rounded-2xl bg-white p-8 text-center shadow-card">
        ...
        <h2 className="text-2xl font-bold text-primary">¡Cita reservada!</h2>
        <p className="mt-2 text-secondary">
          Te enviamos confirmación por WhatsApp (modo demo). Revisa /mis-citas para gestionar tu cita.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl bg-white p-6 shadow-card md:p-8">
      ...
      {step === 2 && (
        <div className="space-y-3">
          <h2 className="text-xl font-semibold text-primary">Elige odontólogo</h2>
          <button
            onClick={() => {
              setDentistId("");
              setStep(3);
      ...
      {step === 3 && (
        <div className="space-y-4">
          ...
          {availability.every((a) => a.slots.length === 0) && (
            <p className="text-sm text-secondary">No hay horarios disponibles este día.</p>
          )}
      ...
      {step === 4 && (
        <div className="space-y-4">
          <h2 className="text-xl font-semibold text-primary">Tus datos</h2>
          <input
